...
class TestVectorStoreSearch:
    ...
    def test_search_with_zero_max_results(
        self, mock_embedding_fn, mock_client, test_config
    ):
        ...
        mock_collection.query.assert_called_once_with(
            query_texts=["test query"], n_results=0, where=None  # This is the bug!
        )
    ...
    @patch("chromadb.PersistentClient")
    @patch("chromadb.utils.embedding_functions.SentenceTransformerEmbeddingFunction")
    def test_search_with_fixed_max_results(
        self, mock_embedding_fn, mock_client, test_config
    ):
        ...
        mock_collection.query.assert_called_once_with(
            query_texts=["test query"], n_results=5, where=None  # This is the fix!
        )
    ...
    @patch("chromadb.PersistentClient")
    @patch("chromadb.utils.embedding_functions.SentenceTransformerEmbeddingFunction")
    def test_search_with_custom_limit_overrides_config(
        self, mock_embedding_fn, mock_client, test_config
    ):
