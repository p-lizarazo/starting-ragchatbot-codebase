...
class TestAIGeneratorWithTools:
    ...
    def test_generate_response_with_tool_use(self, mock_anthropic_class):
        ...
        mock_final_response.content = [Mock(text="Here's what I found about machine learning...")]
    ...
    def test_tool_execution_error_handling(self, mock_anthropic_class):
        ...
        mock_tool_manager.execute_tool.return_value = "Search error: No relevant content found"
...
class TestAIGeneratorExceptionHandling:
    ...
    def test_api_exception_propagation(self, mock_anthropic_class):
        ...
        print("This means exceptions will propagate to RAG system and potentially cause 'query failed'")
    ...
    def test_tool_execution_exception_graceful_handling(self, mock_anthropic_class):
        ...
        mock_fallback_response.content = [Mock(text="I encountered an error with the search tool")]
        
        # Configure client to return initial response, then fallback
        mock_client.messages.create.side_effect = [mock_initial_response, mock_fallback_response]
...
class TestAIGeneratorSequentialToolCalling:
    ...
    def test_sequential_tool_calling_two_rounds(self, mock_anthropic_class):
        ...
        mock_final_response = Mock()
        mock_final_response.stop_reason = "stop"
        mock_final_response.content = [Mock(text="Here's the comparison between the courses...")]
        ...
        mock_tool_manager.execute_tool.assert_any_call("get_course_outline", course_title="Machine Learning")
    ...
    def test_natural_termination_after_one_round(self, mock_anthropic_class):
        ...
        mock_response_2.content = [Mock(text="Based on the search, here's what I found...")]
...
class TestAIGeneratorWithRealToolManager:
    ...
    @patch('anthropic.Anthropic')
    def test_integration_with_real_tool_manager(self, mock_anthropic_class, mock_vector_store, sample_search_results):
        ...
        mock_final_response = Mock()
        mock_final_response.content = [Mock(text="Final response about machine learning")]
    ...
    @patch('anthropic.Anthropic')
    def test_integration_with_buggy_vector_store(self, mock_anthropic_class, mock_vector_store):
        ...
        if len(tool_calls) > 1:
            ...
            if "content" in tool_result_message and isinstance(tool_result_message["content"], list):
